...
Previously: ClassificationContext included agent, model,
first_user_message, part_content, bash_commands, and message_count.

ClassificationContext SHALL include a `project_name` field containing
the project directory name. The `project_name` field SHALL be
available for matching in both classification_rules and budget_rules
conditions. Unknown fields SHALL continue to return undefined (no
match).
...
Previously: The sessions table had columns session_id, project_id,
agent, model, classification, title, started_at, ended_at, metadata.

The sessions table SHALL include a `budget_tag TEXT` column that
stores the budget identifier or NULL. The column SHALL be added via
ALTER TABLE migration when upgrading from schema version 2 to 3.
