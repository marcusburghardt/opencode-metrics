...
Previously: writeMetrics() performed only an UPSERT into the
measurements table.
...
- **WHEN** writeMetrics() is called with 12 metrics
- **THEN** up to 12 delta rows SHALL be inserted into
  measurement_deltas (skipping zero deltas)
- **AND** 12 rows SHALL be upserted into measurements
